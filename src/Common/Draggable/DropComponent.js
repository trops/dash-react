...
export default function DropComponent({ item, id, type = 'layout-widget', children = null, onDropItem, width }) {
    ...
    return (
        <div 
            ref={drop} 
            id={id} 
            className={`drop-component relative cursor-pointer rounded min-w-lg ${width} ${isOverCurrent ? 'opacity-50 border-2 border-yellow-500':'opacity-100 border-2 border-none'} `}
        >
            {children}
