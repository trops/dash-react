...
export default function AlgoliaDropComponent({
    id,
    type,
    children,
    onDropItem,
    ...
    return (
        <div
            ref={drop}
            id={id}
            className={`drop-component relative cursor-pointer h-full ${
                canDrop && !isOver ? "opacity-50" : "opacity-100"
            } rounded min-w-lg max-w-2xl`}
            style={
                canDrop
                    ? { animationDelay: "-.75s", animationDuration: ".25s" }
                    : {}
