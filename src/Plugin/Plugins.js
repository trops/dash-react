...
const PluginsC = ({ navigate }) => {
    ...
    function renderActivePlugins() {
        return activePlugins !== null && activePlugins.map(p => {
            return <MainMenuItem title={p.name} onClick={() => handleUninstallPlugin(p)}/>;
        })
    }

    function handleUninstallPlugin(plugin) {
        console.log(plugin);
        plugins.uninstall([plugin.name]).then(result => {
            console.log(result);
    ...
    return (
        <Workspace>
            <div className="flex flex-col w-full h-full justify-center items-center space-y-4">
                ...
                <div className="flex text-2xl font-bold text-gray-200">Manage Plugins</div>
                <div className="flex flex-row bg-gray-800 p-2 rounded">
                    <form id="install-file">
                    <div className="flex flex-row items-end">
                        <div class="col-8">
                        <label class="form-label">Package file:
                            <input type="file" name="plugin-file" class="form-control" onChange={handleChooseFile}/>
                        </label>
                        </div>
                        <div>
                            <Button onClick={handleClickInstall} title="Install" />
                            <Button onClick={handleClickUninstall} title="Un-Install" />
                        </div>
                    </div>
                    </form>
                </div>
