import { LayoutBuilderGridItem, LayoutGridContainer, LayoutBuilderConfigMenuItem, LayoutBuilderConfigContainerMenuItem } from "@dash/Layout";
...
export const renderLayout = ({ layout, parentKey = 0, workspace, isDraggable = true, debugMode = false, previewMode = false, onClickAdd = null, onClickRemove = null, onClickShrink = null, onClickExpand = null, onChangeDirection = null, onChangeOrder = null, onOpenConfig = null, onOpenEvents = null, onDropItem = null, dashboardId }) => {
    try {

        // Go through each item in the Workspace Layout to render the items.
        return layout !== null && layout !== undefined && layout
            .filter(t => {
            ...
            return hasChildren === 1 && canHaveChildren === true ? (<LayoutGridContainer
                    key={id}
                    id={id}
                    item={childLayout}
                    parent={parent}
                    ...
                    height={height}
                >
                    {id > 0 && renderLayout({ dashboardId, item: childLayout, layout, parentKey: id, debugMode, previewMode, onClickAdd, onClickRemove, onClickShrink, onClickExpand, onChangeDirection, onChangeOrder, order, onOpenConfig, onOpenEvents, onDropItem, workspace, isDraggable })}
                </LayoutGridContainer>
...
export function renderLayoutMenu({ currentWorkspace, parentKey = 0, onClick, onMouseOver }) {
    try {
        ...
        return currentWorkspace !== null && currentWorkspace !== undefined && currentWorkspace['layout']
            ...
            .map((child) => {
                ...
                const hasChildren = 'hasChildren' in child 
                    ? child['hasChildren'] 
                    : 0;

                return hasChildren === 1 ? (
                    <LayoutBuilderConfigContainerMenuItem key={`layout-builder-config-container-${id}`} onClick={onClick} onMouseOver={handleMouseOver} id={id} component={component} item={childLayout}>
                        {id > 0 && renderLayoutMenu({ currentWorkspace, parentKey: id, onClick, onMouseOver })}
                    </LayoutBuilderConfigContainerMenuItem>
                ) : (<LayoutBuilderConfigMenuItem onClick={onClick} onMouseOver={handleMouseOver} id={id} component={component} item={childLayout} />);
        });
...
export function renderComponent(component, id, params = {}, children = null) {
    try {
        if (component) {
            ...
            const WidgetToRender = WidgetFactory.render(component, `widget-${id}`, params, children);
            return WidgetToRender ? (WidgetToRender) : (<div className={`flex flex-col h-full`}>No</div>);
        } else {
            return null;
        }
    } catch(e) {
...
export function isMaxOrderForItem(tempLayout, itemId, parentId) {
    ...
    if (children.length > 0) {
        maxOrder = Math.max(...newTemp.filter(i => i.parent === parentId).map(t => t.order));
    }
...
export function isMinOrderForItem(tempLayout, itemId, parentId) {
    ...
    let maxOrder = 0;
    if (children.length > 0) {
        maxOrder = Math.min(...newTemp.filter(i => i.parent === parentId).map(t => t.order));
    }
    return itemId['order'] === maxOrder;
}
...
export function addItemToItemLayout(tempLayout, id, itemToAdd, hasChildren = false) {
...
export function removeItemFromLayout(tempLayout, id) {
    if (tempLayout.length > 1) {
        ...
        // remove the children...
        indexOfChildren.length > 0 && indexOfChildren.forEach(index => {
 ...
 */
export function getNearestParentWorkspace(workspaceLayout, currentItem, parentItem, count = 0) {
    try {
                ...
                if (currentItem !== null) {
                    ...
                    if (currentItem['type'] === 'workspace' && currentItem['workspace'] !== 'layout') {
                        ...
                        if (currentItem['type'] === 'workspace' && currentItem['workspace'] !== 'layout') {
                            ...
                            const parentItemFromLayout = getLayoutItemById(workspaceLayout, parentId);
                            
                            count++;

                            return parentItemFromLayout !== null 
                                ? getNearestParentWorkspace(workspaceLayout, parentItem, parentItemFromLayout, count)
                                : {};
                        }
                    }
                } else {
