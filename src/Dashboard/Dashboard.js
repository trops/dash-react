...
export const Dashboard = ({ workspace = null, preview = true }) => {
    ...
    const { currentTheme, changeCurrentTheme, themesForApplication } = useContext(ThemeContext);
    ...
    function loadWorkspaces() {
        ...
        // API
        api.workspace.listWorkspacesForApplication(process.env.REACT_APP_APP_ID);
    }
    ...
    function renderMenuItems() {
        return menuItems !== undefined && menuItems.length > 0 && menuItems.map((menuItem, index) => {
            const selected = selectedMainItem !== null ? selectedMainItem.id === menuItem.id : false;
            return (
                <DashboardMenuItem 
    ...
    function handleClickSaveWorkspace() {
        ...
        api.workspace.saveWorkspaceForApplication(process.env.REACT_APP_APP_ID, workspaceToSave);
    }
    ...
    return menuItems && currentTheme && (
        <LayoutContainer className={'flex flex-row h-full p-0 overflow-hidden w-full space-x-0'} height="h-full" width="w-full" direction="row" scrollable={false} space={false}>
            <DndProvider backend={HTML5Backend}>
                <div className={`flex flex-col space-y-1 ${currentTheme['bg-secondary-very-dark']} p-2 items-center} h-full z-40 justify-between`}>
                    <div className="flex flex-col">
                        <div className="w-10 h-10 items-center justify-center">
                            <ButtonIcon icon="home" onClick={() => setWorkspaceSelected(null)} />
                        </div>
                        {menuItems && renderMenuItems()}
                    </div>
                    <div className="flex flex-col">
                        <div className="w-10 h-10 items-center justify-center">
                            <ButtonIcon icon="plus" onClick={handleAddNewMenuItem} hoverBackgroundColor={'hover:bg-green-700'} />
                        </div>
                        <div className="w-10 h-10 items-center justify-center">
                            <ButtonIcon icon="palette" onClick={handleOpenThemeManager} hoverBackgroundColor={'hover:bg-orange-700'} />
                        </div>
                        <div className="w-10 h-10 items-center justify-center">
                            <ButtonIcon icon="computer"  onClick={() => setIsSettingsModalOpen(true)} hoverBackgroundColor={'hover:bg-orange-700'} />
                        </div>
                    </div>
                </div>
                ...
                <AddMenuItemModal open={isAddItemModalOpen} setIsOpen={() => setIsAddWidgetModalOpen(!isAddItemModalOpen)} onSave={handleSaveNewMenuItem} />
                ...
                <ApplicationSettingsModal open={isSettingsModalOpen} setIsOpen={setIsSettingsModalOpen} workspaces={workspaceConfig} />
