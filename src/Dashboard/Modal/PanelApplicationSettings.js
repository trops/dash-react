...
export const PanelApplicationSettings = ({ settings, workspaces, setIsOpen }) => {
            ...
            case "widget":
                if (Object.keys(args).length > 0) {
                    if (args[0] === 'list') {
                        ...
                        const widgets = Object.keys(m).filter(key => m[key]['type'] === 'widget');
                ...
                if (Object.keys(args).length > 0) {
                    ...
                    if (args[0] === 'list') {
                        ...
                        const themeKeys = Object.keys(themes).map(themeKey => {
                            return { name: themes[themeKey]['name'], key: themeKey };//join('\n');
                        });
    ...
    function addDashInput(input, textType = 'string') {
        const applicationInputCurrent = deepCopy(applicationInput);
        applicationInputCurrent.push({ author: 'dash', message: input, textType });
    ...
    function writeUserInput(input) {
        return (
            <div className="flex flex-row shadow w-full">
                <span className="text-blue-500 bg-gray-800 px-4 py-2 rounded w-full shadow text-xs">{input}</span>
            </div>
        );
    }
    ...
    function writeApplicationInput(input, textType = 'string') {
        return textType === 'string' ? (
            <div className="flex flex-row shadow w-full">
                <span className="text-green-500 bg-slate-900 px-4 py-2 rounded w-full shadow text-xs">{input}</span>
            </div>
        ) : (
    ...
    function renderApplicationInput() {
        return applicationInput.map((input, index) => {
            return (
                <div key={`input-${index}`} className="flex flex-row w-full py-1 font-mono text-green font-normal text-xs xl:text-sm">{writeInput(input['message'], input['author'], input['textType'])}</div>
            );
        });
    }
            ...
            <div className="flex flex-row w-full h-full overflow-hidden xl:justify-between xl:space-x-4">
                <div className={`flex-col h-full rounded font-medium w-full hidden xl:flex xl:w-1/3 p-10 justify-between`}>
                    <div className="flex flex-col rounded py-10 space-y-4 w-full overflow-hidden">
                        ...
                        <SubHeading3 title={"01001000 01100101 01101100 01101100 01101111"} padding={false} />
                        {/* <div className="flex flex-col h-full overflow-y-scroll">
                            <pre className="text-xs">{JSON.stringify(applicationInput, null, 2)}</pre>
                        </div> */}
                    </div>
                </div>
                <div className={`flex flex-col h-full rounded xl:rounded-0 w-full lg:w-full`}>
                    <div className="flex flex-col bg-gradient-to-tr from-gray-900 to-gray-800 text-green-600 h-full w-full rounded-lg p-6 overflow-hidden border border-gray-900">
                        ...
                        <div className="text-xs text-gray-400 space-y-1">
                            <div className="flex flex-row text-xs text-gray-400">Type 'help' for more information</div>
                            <InputText name="name" padding={'p-4'} value={userInput} onKeyDown={handleKeyDown} onChange={handleUserInput} textSize={'text-lg'} placeholder="" bgColor={'bg-gray-400'} textColor={'text-gray-800'} hasBorder={false} />
                        </div>
                    </div>
                </div>
