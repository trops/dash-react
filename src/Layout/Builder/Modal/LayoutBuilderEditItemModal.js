...
export const LayoutBuilderEditItemModal = ({ workspace, open, setIsOpen, onUpdate, item = null }) => {
    ...
    function handleUpdate(data) {
        ...
        const newLayout = replaceItemInLayout(workspaceTemp.layout, data['id'], data);
            ...
            if (itemSelected !== null && workspaceSelected !== null) {
                ...
                if (itemSelected.parentWorkspace !== undefined && itemSelected.parentWorkspace !== null) {
                    ...
                    if (itemSelected['workspace'] === parentWorkspaceTemp['workspace']) {
                        ...
                        return itemSelected.parentWorkspace && renderLayout({ workspaceSelected, layout, parentKey: 0, previewMode: true, isDraggable: false });
                    } else {
                        // workspace mismatch! 
                        return null;
                    }
    ...
    return itemSelected && (
            <Modal isOpen={open} setIsOpen={setIsOpen} width={'w-5/6 2xl:w-3/4'} height="h-5/6">
                <Panel>
                <div className={`flex flex-col w-full h-full space-y-2 overflow-hidden p-4`}>
                    <div className='flex flex-row text-xl font-bold text-white justify-between'>
                        <div className='flex flex-row text-xl font-bold text-white p-2 space-x-2 justify-center items-center'>
                            {itemSelected && (
                                <Fragment><FontAwesomeIcon icon="folder" /><span className="text-xl font-bold text-gray-200">{getTitle()}</span></Fragment>
                            )}
                        </div>
                        <ButtonIcon icon={"xmark"} onClick={() => setIsOpen(false)} bgColor={`${getContainerColor(itemSelected['parentWorkspace'])}`} />
                    </div>
                    <div className='flex flex-row w-full h-full space-x-4 overflow-hidden'>
                        <div className='flex flex-col w-3/4 min-w-3/4 bg-gray-900 h-full rounded p-2'>
                            ...
                            <div className="flex flex-col w-full h-full border-2 border-gray-800 rounded space-y-2">
                                <div className="flex bg-gray-800 p-2 text-xs text-gray-300 rounded-br uppercase font-bold">Preview</div>
                                <div className="flex flex-col p-2">
                                    {itemSelected !== null && workspaceSelected !== null && renderEditContainer()}
                                </div>
                            </div>
                        </div>
                        {item && (
