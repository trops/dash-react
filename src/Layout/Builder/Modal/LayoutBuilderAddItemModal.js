...
import { Button, Panel, Heading, SubHeading3, Paragraph, MenuItem3, Modal, InputText } from "@dash/Common";
import { renderLayout, getNextHighestId, getNextHighestOrder, getBorderStyle  } from "@dash/Utils";
...
export const LayoutBuilderAddItemModal = ({ workspace, open, setIsOpen, item = null, onSaveItem = null }) => {
    ...
    useEffect(() => {
        if (open === false) {
            ...
            if (workspaceSelected !== workspace) setWorkspaceSelected(() => workspace);
        }
    }, [open]);
    ...
    function renderWidgets() {
        ...
        const parentWorkspaceType = item['parentWorkspaceName'] !== null && item['parentWorkspaceName'] !== undefined
            ? item['parentWorkspaceName'] 
            : 'layout';

        if (parentWorkspaceType !== null) {
            const options = workspaceType !== null && canAddChildren && Object.keys(componentMap)
                .sort()
                .filter(c => componentMap[c]['type'] === 'widget')
                .filter(c => workspaceType !== null ? (componentMap[c]['workspace'] === parentWorkspaceType) : true)
    ...
    function renderWorkspaces() {
        ...
        const options = canAddChildren === true && item['parentWorkspaceName'] === 'layout'
            ? Object.keys(componentMap)
                .sort()
                .filter(i => searchTerm !== '' ? i.toLowerCase().includes(searchTerm) : true)
                .filter(c => componentMap[c]['type'] === 'workspace' || componentMap[c]['workspace'] === 'layout')
    ...
    function handleClickItem(data) {
        try {
            ...
            layoutModel['parent'] = parentWorkspace !== null && parentWorkspace !== undefined 
                ? ('id' in parentWorkspace ? parentWorkspace['id'] : 0 ) 
    ...
    function renderMenuItem(type, componentName) {
        return (
            <MenuItem3 onClick={() => handleClickItem({ type, component: componentName })}>
                {componentName}
            </MenuItem3>
        )
    }
    ...
    function renderAddContainer(itemData) {
        try {
            const workspaceSelectedTemp = JSON.parse(JSON.stringify(workspaceSelected));
            ...
            if (item.parentWorkspace !== undefined && item.parentWorkspace !== null) {
                ...
                let parentWorkspaceTemp = JSON.parse(JSON.stringify(item.parentWorkspace));
                let layout = JSON.parse(JSON.stringify(workspaceSelectedTemp['layout']));
                ...
                return item.parentWorkspace && renderLayout({ workspaceSelected, layout, parentKey: item.parentWorkspace['parent'], previewMode: true, isDraggable: false });
            }
        } catch(e) {
            console.log(e);
        }
    ...
    return item && (
            <Modal isOpen={open} setIsOpen={setIsOpen} width={'w-5/6'} height={'h-5/6'}>
                ...
                <div className={`flex flex-col w-full h-full overflow-hidden`}>
                    <div className='flex flex-row w-full h-full space-x-4 overflow-hidden rounded'>
                        <div className='flex flex-col h-full rounded p-4 text-gray-200 overflow-y-scroll w-1/4 space-y-4'>
                            ...
                            <div className="flex flex-row pb-4">
                                <InputText textSize="text-sm" onChange={(e) => setSearchTerm(e.target.value)} value={searchTerm} placeholder="Widgetize" />
                            </div>
                            <div className="flex flex-col space-y-2">
                                <span className="text-xs uppercase font-bold px-2 text-gray-400">Layout/Function</span>
                                <div className="flex flex-col rounded space-y-2">
                                    {renderWorkspaces()}
                                </div>
                            </div>
                            <div className="flex flex-col space-y-2">
                                <span className="text-xs uppercase font-bold px-2 text-gray-400">Widgets</span>
                                <div className="flex flex-col rounded space-y-2">
                                    {renderWidgets()}
                                </div>
                            </div>
                        ...
                        <div className={`flex flex-row h-full text-gray-200 overflow-y-scroll w-full rounded p-4 space-x-4 ${theme['bg-secondary-dark']}`}>
                            {/* render the widget item here. */}
                                 ...
                                 <div className='flex-col h-full rounded font-medium text-gray-400 w-full xl:w-1/2 p-10'>
                                    {/* render the widget item here. */}
                                     <div className="flex flex-col rounded p-4 py-10 space-y-4">
                                        ...
                                        <SubHeading3 title={'Choose a Workspace or Widget from the Available Components.'} padding={false} />
                                        <Paragraph text={'Don\'t worry, you can\'t mess this up.'} padding={false} />
                                     </div> 
                                </div>
                            )}
                            {menuItemSelected !== null && (
                                <div className={`flex flex-col rounded border-2 border-gray-800 ${getBorderStyle(menuItemSelected)} overflow-hidden h-full w-3/4 bg-gray-900`}>
                                    <div className={`flex flex-col p-2 space-x-1 uppercase text-xs text-gray-200 font-bold bg-gray-800`}>
                                        {menuItemSelected !== null && (<div className="flex flex-row">Preview: {menuItemSelected['component']}</div>)}
                                    </div>
                                    <div className="flex flex-col overflow-hidden justify-between h-full">
                                        <div className="flex flex-col grow p-2">
                                            {renderAddContainer(menuItemSelected)}
                                        </div>
                                    </div>
                                </div>
                            )}
                    ...
                    <div className={`flex flex-row justify-end ${theme['bg-primary-very-dark']} p-4 rounded-br rounded-bl border-t ${theme['border-primary-dark']}`}>
                        <div className="flex flex-row space-x-2">
                            <Button title={'Cancel'} bgColor={'bg-gray-800'} textSize={'text-lg'} padding={'py-2 px-4'} onClick={() => setIsOpen(false)} />
                            <Button title={'Save Changes'} bgColor={'bg-gray-800'} hoverBackgroundColor={'hover:bg-green-700'} textSize={'text-lg'} padding={'py-2 px-4'} onClick={handleAddItem} />
                        </div>
                    </div>
                </div>
            </Panel>
