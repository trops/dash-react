        ...
        try {
            ...
            if (itemSelected !== null && workspaceSelected !== null) {
                ...
                if (itemSelected.parentWorkspace !== undefined && itemSelected.parentWorkspace !== null) {
                    ...
                    let parentWorkspaceTemp = JSON.parse(JSON.stringify(itemSelected.parentWorkspace));
                    ...
                    if (itemSelected['workspace'] === parentWorkspaceTemp['workspace']) {
                        ...
                        return itemSelected.parentWorkspace && renderLayout({ workspace: workspaceSelected, layout, parentKey: 0, previewMode: true, isDraggable: false });
                    } else {
                        // workspace mismatch! 
                        return null;
                    }
        ...
        <Panel>
            <div className='flex flex-row w-full h-full space-x-4 overflow-hidden'>
                <div className='flex flex-col w-3/4 min-w-3/4 h-full rounded'>
                    ...
                    <div className={`flex flex-col w-full h-full rounded space-y-2 border-2 border-dashed ${theme['border-secondary-very-dark']}`}>
                        <div className={`flex p-2 text-xs ${theme['text-secondary-dark']} rounded-br uppercase font-bold `}>Preview</div>
                        <div className={`flex flex-col p-4`}>
                            {itemSelected !== null && workspaceSelected !== null && renderEditContainer()}
                        </div>
                    </div>
                </div>
                <div className={`flex flex-col w-1/4 ${theme['bg-secondary-dark']}`}>
                    {itemSelected && (
                        <WidgetConfigPanel 
                            item={itemSelected}
