...
import { addItemToItemLayout, changeDirectionForLayoutItem, getNextHighestItemInLayout, getNextLowestItemInLayout, removeItemFromLayout, updateLayoutItem, updateParentForItem } from "@dash/Utils";
import { LayoutContainer } from '@dash/Layout';
import {
    LayoutDragBuilder,
} from '@dash/Layout/Builder';
import {
...
export const LayoutBuilder = ({ workspace, preview = false, onTogglePreview, onWorkspaceChange = null, dashboardId }) => {
    ...
    function handleClickConfirmAdd(itemChosen, toItem) {
        ...
        const newLayout = addItemToItemLayout(layout, toItem['id'], itemChosen, hasChildren);
    ...
    function onChangeOrder(item, direction) {
        ...
        let nextItem = null;
        let layoutFiltered = {};
        Object.keys(layout).filter(li => layout[li]['parent'] === item['parent']).forEach(fli => {
    ...
    return (
        <div className={`flex flex-col w-full h-full overflow-hidden p-2`}>
            <div className="flex flex-row w-full h-full overflow-hidden p-2 space-x-2">
                <LayoutContainer id="search-layout-builder" scrollable={true} direction={'col'} width={'w-full'} height={'h-full'} className={'overflow-x-hidden'}>
                    <LayoutDragBuilder
