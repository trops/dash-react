...
export default function LayoutBuilderConfigPanel({ workspace, onComplete, onClickEdit }) {
          ...
          <div className="flex flex-col justify-between h-full overflow-hidden w-full">
              <div className="flex flex-col h-full overflow-hidden w-full rounded space-y-4">
                    ...
                    <div className="rounded flex flex-col">
                      <InputText name={'name'} value={currentWorkspace.name} onChange={handleTextChange} />
                    </div>
                    
                    <div className="flex flex-col overflow-y-auto">
                        {renderLayoutMenu({currentWorkspace, parentKey:0, onClick: handleClickMenuItem, onMouseOver: handleHoverMenuItem, onMouseOut: handleMouseOutMenuItem })}
                    </div>
              </div>

              <div className="flex flex-row w-full p-2">
                  <Button title="Save Changes" onClick={() => onComplete(currentWorkspace)} hoverBackgroundColor="hover:bg-indigo-600" block />
              </div>
