...
import { Button, Heading, SubHeading3, Panel, Modal, InputText } from "@dash/Common";
...
export const AddMenuItemModal = ({ menuItems, open, setIsOpen, onSave}) => {
    ...
    function renderAvailableIcons() {
        const icons = ['phone','clone','home','plug','magnifying-glass','arrow-down','arrow-left', 'arrow-right','arrow-up','minus','arrows-up-down','arrows-left-right','square','eye','pencil','folder', 'signal','hammer','seedling', 'trophy', 'robot', 'leaf', 'baby', 'baby-carriage', 'database'];
        return icons.map(icon => {
            const selected = icon === menuIconSelected;
            return (
                <div 
                    key={`icon-${icon}`}
                    className={`flex flex-col text-5xl p-4 ${selected === true ? `${theme['bg-secondary-very-dark']} ${theme['border-secondary-very-dark']}`: theme['bg-secondary-medium']} h-fit w-full rounded border-4 ${theme['border-secondary-medium']} ${selected === false && `${theme['hover-bg-secondary-medium']} ${theme['hover-border-secondary-dark']}`} cursor-pointer text-gray-200`}
                    ...
                    <div className='flex flex-col w-full h-full overflow-hidden'>
                        <div className="flex flex-row w-full h-full space-x-4 overflow-hidden p-6">
                            <div className='flex flex-col flex-shrink h-full rounded font-medium text-gray-400 w-1/3'>
                                ...
                                {menuItemsSelected !== null && (
                                    <div className="flex flex-col rounded p-6 py-10 space-y-4">
                                        ...
                                        <SubHeading3 title={`Add new "folders" to organize all of your dashboards.`} padding={false} />
                                    </div>
                                )}
                            </div>
                           ...
                           <div className="flex flex-col w-2/3 space-y-4 py-10">
                                <div className="flex flex-col rounded w-full">
                                    <InputText value={menuItemNameSelected} onChange={handleMenuNameChange} placeholder="My Folder" />
                                </div>
                        ...
                        <div className="flex flex-row justify-between bg-gray-900 p-4 rounded-br rounded-bl border-t border-gray-800">
                            <div className="flex flex-row text-lg text-gray-600 items-center font-bold px-4">
                                Click the <FontAwesomeIcon icon="plus" className="px-2"/> button to open this window.
                            </div>
