...
const MainMenuConst = ({ onClick = null, active, menuItems, workspaces, selectedMainItem = null, onWorkspaceMenuChange  }) => {
    ...
    function renderPluginMenuItems() {
        return menuItems.map(item => {
            return (<MainMenuItem key={`main-menu-item-${item.id}`} onClick={(e) => handleClickMenuItem(item)} title={item.label} />);
        })
    }
    ...
    function renderWorkspaces (workspaces) {
        ...
        return workspaces && menuItems
            ...
            .map(menuItem => {
                ...
                const folderSelected = selectedMainItem !== null ? menuItem.id === selectedMainItem.id : false;
                return (
                <div className={`${folderSelected && 'rounded'}`} key={`menu-item-${menuItem.id}`}>
                    <div className={`flex flex-row justify-between border-b ${currentTheme['border-secondary-medium']} mb-2 p-2`}>
                        <div className="flex flex-row text-xs items-center">
                            <FontAwesomeIcon icon={menuItem.icon} />
                            <span className="p-2 uppercase font-bold">{menuItem.name}</span>
                        </div>
                        <ButtonIcon icon="plus" textSize={'text-xs'} onClick={() => handleCreateNew(menuItem)} />
                    </div>
                    <div className="flex flex-col pb-4 space-y-1">
                        {workspaces
                            .filter(w => 'menuId' in w && w.menuId === menuItem.id)
                            .filter(ws => searchTerm !== '' ? ws.name.toLowerCase().includes(searchTerm.toLowerCase()) : true)
                            .map(ws => (
                                <MainMenuItem
                                    ...
                                    onClick={(e) => handleClickMenuItem(ws)} title={ws.name} onDropItem={(e) => handleDropMenuItem(e)} 
                                />
                            )
                        )}
                    </div>
        ...
        return workspaces && (
            <div key={`menu-item-orphan`}>
                {selectedMainItem === null && (
                    <div className={`flex flex-row justify-between border-b border-blue-700 mb-2 p-2 ${currentTheme['textSecondary']}`}>
                        <div className="flex flex-row text-xs items-center">
                            <FontAwesomeIcon icon={"folder"} />
                            <span className="p-2 uppercase font-bold">Uncategorized</span>
                        </div>
                    </div>
                )}
                <div className="flex flex-col pb-4 space-y-1">
                    {workspaces
                        .filter(mi => searchTerm !== '' ? true : (selectedMainItem !== null ? mi.menuId === selectedMainItem.id : true))
    ...
    function handleDropMenuItem(dropData) {
        ...
        if (workspaceSelected) {
            ...
            api.workspace.saveWorkspaceForApplication(process.env.REACT_APP_APP_ID, newWorkspace);
        }
    }
    ...
    function handleCreateNew(menuItem) {
        ...
        onClick && onClick({ id: Date.now(), name: 'New Workspace', label: 'New', type: selectedMainItem, layout: newLayout, menuId: menuItem['id'] });
    }
        ...
        <div className="flex flex-col min-w-64 w-64 h-screen">
            <div className="flex flex-col space-y-2 w-full h-full">
                <div className="flex flex-row justify-between">
                    <InputText name="search-workspaces" value={searchTerm} placeholder="Search Workspaces" onChange={handleChangeSearch} textSize="text-sm" />
                </div>
