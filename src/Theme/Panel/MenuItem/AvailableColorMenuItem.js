...
const AvailableColorMenuItem = ({ colorName, variantName, onClick }) => {
    ...
    const stringColor = `bg-${colorName}${variantName !== null ? `-${variantName}` : ''}`;
    return (
        <div 
        ...
        >
            <div className="text-sm w-full justify-start">{colorName} {variantName}</div>
            <div className={`flex flex-row text-xs w-full justify-start text-gray-600 font-normal`}>{stringColor}</div>
        </div>
    )
}
