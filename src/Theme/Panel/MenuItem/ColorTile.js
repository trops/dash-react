...
const ColorTile = ({ 
    ...
    const c = ColorModel({ colorFromTheme, colorName, colorType, shade, variant, level: colorLevelName, ...rest });
    ...
    return (
        <div 
            className={`flex flex-col rounded-lg cursor-pointer items-center justify-center border-2 ${selected === true ? "border-yellow-500": "border-gray-800"} hover:border-yellow-500 border-gray-800 ${c.class} ${width} ${height}`}
            onClick={() => onClick !== null ? onClick({ ...c, ...rest }) : null}
            onMouseOver={() => onMouseOver !== null ? onMouseOver({ ...c, ...rest }) : null}
        >{c.hex[shade]}</div>
    );
};
