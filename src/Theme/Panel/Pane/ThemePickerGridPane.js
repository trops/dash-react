...
const ThemePickerGridPane = ({ themeKey, onChooseTheme }) => {
    ...
    function renderMenuItem(tk) {
        ...
        return colors.map(color => {
            return <div className={`rounded ${color['color']} h-20 w-full`}></div>
        });
    }
    ...
    function renderCurrentThemes() {
        return Object.keys(themes).map(tk => {
            ...
            return (
                <div 
                    key={`icon-${tk}`}
                    className={`flex flex-col text-xs p-4 space-y-4 h-48 w-full rounded justify-between border-2 ${selected === true ? 'border-yellow-600 hover:border-yellow-600':'hover:border-yellow-600 border-gray-800'} cursor-pointer text-gray-200 ${themes[tk][themeVariant]['bg-primary-dark']}`}
                    onClick={() => onChooseTheme(tk)}
                >
                    <div className="flex flex-col w-full">
                        <span className="font-bold text-xl word-break-all">{current['name']}</span>
    ...
    return (
        <ThemePane>
            <div className="flex flex-row rounded overflow-hidden justify-center items-center align-center w-full">
                <div className="grid grid-cols-3 gap-4 w-full h-full overflow-y-scroll">
                    {renderCurrentThemes()}
                </div>
            </div>
        </ThemePane>
