...
const ThemesPane = ({ themeKey, onChooseTheme, onChooseVariant, onClickNewTheme }) => {
        ...
        <div className='flex flex-col rounded font-medium w-1/4 max-w-1/3 justify-between'>
            <div className='flex flex-col rounded font-medium justify-between overflow-hidden'>
                ...
                <div className="flex flex-col rounded p-6 py-10 space-y-4 w-full hidden xl:flex">
                    <div className="flex flex-row">
                        <Heading title={'Color.'} padding={false} textColor={'text-gray-300'} />
                        <ButtonIcon icon="plus" textSize="text-2xl" backgroundColor={'bg-gray-800'} hoverBackgroundColor={"hover:bg-green-600"} iconSize="h-6 w-6" textColor={"text-gray-400"} onClick={onClickNewTheme} />
                    </div>
                    <p className={`text-lg font-normal text-gray-300`}>We all know dark is best, but if you <span className="italic">have</span> to change colors, we'll allow it.</p>
                </div>
            ...
            <div className="flex flex-col space-y-2 w-full">
                <Toggle 
                    ...
                    setEnabled={() => onChooseVariant(themeVariant === 'dark' ? 'light':'dark')} 
                    text={'Dark'}
                    theme={false}
