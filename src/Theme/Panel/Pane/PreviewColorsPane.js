...
const PreviewColorsPane = ({ styles = null, theme, itemType = null, onClickItem = null, onResetStyles = null }) => {
    ...
    function renderAvailableColors() {
        ...
        return Object.keys(newStyles)
            ...
            .map(key => {
                ...
                const parts = key in newStyles 
                    ? (newStyles[key] !== undefined ? newStyles[key].split('-') : null)
                    : null;
                    ...
                    return key !== 'string' && (
                        <div className="flex flex-row justify-between py-2 items-center border-b border-gray-700 px-2">
                            ...
                            <ColorTile 
                                ...
                                objectType={'bg'} 
                                variant={'dark'}
                                onClick={(data) => handleClickItem(data, key, itemType, objectType)}
                            />
                        </div>
                    )
                }
        ...
        <ThemePane>
            ...
            {hasCustomStyles() === true && (
                <div className="flex flex-row justify-end">
                    <Tag3 theme={false} text={'Reset to Default'} backgroundColor={'bg-orange-700'} onClick={handleResetStyles} textSize={'text-xs'} />
                </div>
            )}
        </ThemePane>
    ) : null;
