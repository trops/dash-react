...
const ThemeMenuPane = ({ theme, onChooseColor, onChooseReplacementColor }) => {
    ...
    function handleReplaceColor({ panelType = 'main', colorType, colorName, shade = 500 }) {
    ...
    function handleReplaceColorTemp({ panelType = 'main', colorType, colorName, shade = 500 }) {
        if (selectedColor !== null) {
            const colorReplacement = { colorName, colorType, shade, panelType };
            const r = ColorModel(colorReplacement);
            onChooseReplacementColor(selectedColor, r);
    ...
    return (
        <ThemePane className={'space-y-2'}>
            {(selectedColor === null || selectedColor['color']['panelType'] === 'main') && (
                <div className="flex flex-col rounded w-full bg-gray-800 space-y-2">
                    <div className="flex flex-row text-xs uppercase font-bold w-full text-gray-200 bg-gray-900 p-2 rounded-t border-b border-gray-700">Main</div>
                    <div className="flex flex-row w-full space-x-2 p-4">
                        {colorTypes
                            .filter(ct => selectedColor !== null ? (selectedColor['color']['panelType'] === 'main' && selectedColor['color']['colorType'] === ct) : true)
                            .map(colorType => {
            ...
            {(selectedColor === null || selectedColor['color']['panelType'] === 'sub') && (
                <div className={`flex flex-col rounded w-full space-y-2 min-h-1/4 overflow-y-scroll ${selectedColor !== null ? 'h-1/4':'h-full'}`}>
                    {colorTypes
                        .filter(ct => selectedColor !== null ? (selectedColor['color']['panelType'] === 'sub' && selectedColor['color']['colorType'] === ct) : true)
                        .map(colorType => {
                        return (
                            <div className="flex flex-col w-full h-full rounded bg-gray-800">
                                <div className="flex flex-row text-xs uppercase font-bold w-full text-gray-200 bg-gray-900 p-2 rounded-t border-b border-gray-700">{colorType}</div>
                                <div className="flex flex-col p-2">
                                    {themeVariants
                                        .filter(v => selectedColor !== null ? (selectedColor['color']['level'] === v) : true)
                                        .map(colorLevelName => {
                                            ...
                                            const selected = selectedColor !== null 
                                                ? colorType === selectedColor['color']['colorType'] && colorLevelName === selectedColor['color']['level'] && selectedColor['color']['panelType'] === 'sub' 
                                                : false;
                                            return (
                                                <div className="flex flex-row justify-between py-2 items-center border-b border-gray-700 px-2">
                                                    <span className="text-sm font-bold text-gray-300">{colorLevelName}</span>
                                                    <ColorTile 
