...
export const WidgetApi = {
     ...
     */
    storeData: function(data, { filename = null, callbackComplete = null, callbackError = null }) {
        // set the filename
        ...
        if (eApi) {
            ...
            if (callbackComplete !== null) {
                eApi.on(eApi.events.DATA_SAVE_TO_FILE_COMPLETE, (e, message) => callbackComplete(e, message));
            }
            if (callbackError !== null) {
                eApi.on(eApi.events.DATA_SAVE_TO_FILE_ERROR, (e, message) => callbackError(e, message));
            }
            // request.
     ...
     */
    readData: function({ filename = null, callbackComplete = null, callbackError = null }) {
        try {
            const toFilename = filename !== null ? filename : `${this.uuid()}.json`;
            const eApi = this.electronApi();
            eApi.removeAllListeners();
            if (callbackComplete !== null) {
                eApi.on(eApi.events.DATA_READ_FROM_FILE_COMPLETE, (e, message) => callbackComplete(e, message));
            }
            if (callbackError !== null) {
                callbackError !== null && eApi.on(eApi.events.DATA_READ_FROM_FILE_ERROR, (e, message) => callbackError(e, message));
            }
